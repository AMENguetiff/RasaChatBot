...
FORM_DESCRIPTION = {
    "cc_payment_form": "payement vers une carte de credit",
    "transfer_money_form": "transfert de l'argent",
    "transaction_search_form": "recherche des operation",
}

class Actionaddinglink(Action):
    def name(self) -> Text:
        return "action_dynamic_link"
    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        Lien="http://www.stb.com.tn/uploads/content/demande-carte-stb.pdf"
        print("Link: ",tracker.get_slot('LINK'))
        text=tracker.latest_message['text']
        print(text)
        dispatcher.utter_template("utter_order_CIBcard",tracker,lien=Lien)
        return []
...
class ValidatePayCCForm(CustomFormValidationAction):
    """valider les slots du cc_payment_form"""
    ...
    async def validate_amount_of_money(
        ...
        account_balance = profile_db.get_account_balance(tracker.sender_id)
        #vérifier si l'utilisateur a demandé de payer le solde total ou le solde minimum
            ...
            if value and value.lower() in balance_types:
                ...
                if not credit_card:
                    dispatcher.utter_message(
                        f"Je vois que vous voulez payer le {balance_type}."
                    )
    ...
    async def validate_credit_card(
        ...
        """Valider la valeur su 'credit_card' slot"""
    ...
    async def explain_credit_card(
        ...
        """explique 'credit_card' slot"""
        dispatcher.utter_message("Vous avez les cartes de crédit suivantes:")
...
class ActionTransactionSearch(Action):
    ...
    async def run(
        ...
        if tracker.get_slot("zz_confirm_form") == "yes":
            ...
            vendor_name = f" avec {vendor.title()}" if vendor else ""
...
class ValidateTransactionSearchForm(CustomFormValidationAction):
    """Valider les Slots de transaction_search_form"""
...
class ValidateTransferMoneyForm(CustomFormValidationAction):
    """Valider les Slots de transfer_money_form"""
    ...
    async def validate_PERSON(
        ...
        """Valider la valuer du 'PERSON' slot"""
...
class ActionShowBalance(Action):
    """Affiche le solde des comptes bancaires ou de carte de crédit"""
    ...
    async def run(
        ...
        if active_form_name:
            # gardez le tracker propre pour les prédictions avec des histoires de changement de formulaire
            events.append(UserUtteranceReverted())
            # déclencher utter_ask_{form}_AA_CONTINUE_FORM, en en faisant le request_slot
            events.append(SlotSet("AA_CONTINUE_FORM", None))
            # éviter que le bot passe en mode écoute après UserUtteranceReverted
...
class ActionShowRecipients(Action):
    """Répertorie le contenu du slotknown_recipients"""
...
class ActionAskTransactionSearchFormConfirm(Action):
    """Demande le slot 'zz_confirm_form' de 'transaction_search_form'
    Une action personnalisée est utilisée à la place d'une réponse 'utter_ask' car une autre
    la question est posée en fonction des emplacements 'search_type' et 'vendor_name'.
    """
    ...
    async def run(
        ...
        if search_type == "spend":
            text = (
                f"Voulez-vous chercher des transactions{vendor_name} entre "
                f"{start_time_formatted} et {end_time_formatted}?"
            )
        elif search_type == "deposit":
            text = (
                f"Voulez-vous savoir les dépôts effectués sur votre compte entre "
                f"{start_time_formatted} et {end_time_formatted}?"
            )
        buttons = [
            {"payload": "/affirm", "title": "Oui"},
            {"payload": "/deny", "title": "Non"},
        ]
...
class ActionSwitchFormsAsk(Action):
    ...
    async def run(
        ...
        if (
            ...
            text = (
                f"Nous n'avons pas encore terminé le {FORM_DESCRIPTION[active_form_name]}. "
                f"Vous etes sur de passer à {FORM_DESCRIPTION[next_form_name]}?"
            )
            buttons = [
                {"payload": "/affirm", "title": "Oui"},
                {"payload": "/deny", "title": "Non"},
...
class ActionSwitchFormsDeny(Action):
    ...
    async def run(
        ...
        if active_form_name not in FORM_DESCRIPTION.keys():
            ...
            text = f"D'accord, continuons avec le {FORM_DESCRIPTION[active_form_name]}."
...
class ActionSwitchFormsAffirm(Action):
    ...
    async def run(
        ...
        if (
            ...
            text = (
                f"Parfait. Passons du {FORM_DESCRIPTION[active_form_name]} "
                f"au {FORM_DESCRIPTION[next_form_name]}. "
                f"Une fois terminé, vous aurez la possibilité de revenir à la tache precedente."
            )
            dispatcher.utter_message(text=text)
...
class ActionSwitchBackAsk(Action):
    ...
    async def run(
        ...
        if previous_form_name not in FORM_DESCRIPTION.keys():
            ...
            text = (
                f"Souhaitez-vous retourner au "
                f"{FORM_DESCRIPTION[previous_form_name]} maintenant?."
            )
            buttons = [
                {"payload": "/affirm", "title": "Oui"},
                {"payload": "/deny", "title": "Non"},
