...
class Account(Base):
    """Tableau des comptes.
    `session_id` n'a de sens que pour les comptes générés par les sessions de conversation,
    lorsqu'il est égal à `tracker.sender_id`.
    Étant donné que `id` s'auto-incrémente, il est utilisé pour générer des numéros de compte uniques en
    en y ajoutant des zéros non significatifs.
    """
...
class Transaction(Base):
    """Transactions table. `to/from_acount_number` are `Account.id` avec des zéros non significatifs"""
...
class ProfileDB:
    ...
    def get_account(self, id: int):
        """Get an `Account` objet basé sur un `Account.id`"""
        return self.session.query(Account).filter(Account.id == id).first()

    def get_account_from_session_id(self, session_id: Text):
        """Get an `Account` objet basé sur un `Account.session_id`"""
        # si le serveur d'action redémarre au milieu d'une conversation, la base de données devra être repeuplée en dehors d'une action_session_start
    ...
    def get_account_number(account: Union[CreditCard, Account]):
        """Obtenez un numéro de compte bancaire ou de carte de crédit en ajoutant le nombre approprié de zéros non significatifs à un `Account.id`"""
    ...
    def get_account_from_number(self, account_number: Text):
        """Obtenez un compte bancaire ou de carte de crédit basé sur un numéro de compte"""
    ...
    def get_recipient_from_name(self, session_id: Text, recipient_name: Text):
        """ Obtenez le destinataire en fonction de son nom.
        en prenant le premier s'il y en a plusieurs qui correspondent.
        """
    ...
    def list_known_recipients(self, session_id: Text):
        """ Répertorier les surnoms de destinataires disponibles pour un titulaire de compte"""
        recipients = (
    ...
    def check_session_id_exists(self, session_id: Text):
        """Check if an account for `session_id` deja existe"""
    ...
    def search_transactions(
        ...
        """Trouvez toutes les transactions pour un compte entre `start_time` et `end_time`.
        Recherche les transactions de dépenses par défaut, définissez « deposit » sur « True » pour rechercher les revenus.
        Recherche les transactions avec n'importe qui par défaut, définissez « vendeur » pour rechercher par fournisseur
        """
        account = self.get_account_from_session_id(session_id)
    ...
    def list_credit_cards(self, session_id: Text):
        """Liste des cartes de crédit valides pour un compte"""
        account = self.get_account_from_session_id(session_id)
        cards = (
            self.session.query(CreditCard)
    ...
    def get_credit_card(self, session_id: Text, credit_card_name: Text):
        """avoir  `CreditCard` objet basé sur le nom de la carte et `session_id`"""
        account = self.get_account_from_session_id(session_id)
        return (
            self.session.query(CreditCard)
            .filter(CreditCard.account_id == account.id)
    ...
    def get_credit_card_balance(
        ...
        """Obtenez le solde d'une carte de crédit en fonction de son nom et du type de solde"""
    ...
    def add_session_account(self, session_id: Text, name: Optional[Text] = ""):
        ...
        self.session.add(
            Account(session_id=session_id, account_holder_name=name, currency="DT")
        )

    def add_credit_cards(self, session_id: Text):
        """Populate the creditcard table for a given session_id"""
        credit_card_names = ["mastercard", "visa", "cib", "c cash"]
        credit_cards = [
            CreditCard(
    ...
    def populate_profile_db(self, session_id: Text):
        """Initialisez la base de données pour une session de conversation.
        Remplira toutes les tables avec des exemples de valeurs.
        Si les comptes généraux ont déjà été renseignés, il
        ajouter des valeurs spécifiques au titulaire de compte aux tables.
        """
    ...
    def transact(
        ...
        """Ajouter une operation dans la table des transactions"""
